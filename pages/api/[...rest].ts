...
const restHandler: NextApiHandler = async (req, res) => {
  const accessResult = await getAccessToken(req, res)
  console.log(accessResult.accessToken)
  try {
    ...
    return res.status(statusCode).json({
      isSuccess: false,
      errorMessages: [requestError?.statusText ?? 'Internal server error'],
      statusCode: requestError.status ?? 500,
      result: null,
    })
  }
}
